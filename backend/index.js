...
// Try to load config keys with error handling
let configKeys;
let client;
let index;
try {
	configKeys = require("./configKeys");
	client = algoliasearch(configKeys.algoliaKeys.appKey, configKeys.algoliaKeys.adminKey);
	index = client.initIndex(configKeys.algoliaKeys.index);
} catch (error) {
	console.log("Warning: Could not load configKeys or initialize Algolia client:", error.message);
	// Create mock objects to prevent crashes
	client = { initIndex: () => ({ search: () => Promise.resolve({ hits: [] }) }) };
	index = { search: () => Promise.resolve({ hits: [] }) };
	configKeys = { dailyApiKey: "placeholder", algoliaKeys: { appKey: "placeholder", adminKey: "placeholder", index: "placeholder" } };
}
...
app.get('/health', (req, res) => {
	res.json({ status: 'OK', message: 'ChatPlus Backend is running' });
});
			...
			if (call.otherUserJoined === false && call.isCaller) {
				...
				try {
					fetch("https://api.daily.co/v1/rooms/" + call.room.name, {
						headers: {
							Authorization: `Bearer ${configKeys.dailyApiKey}`,
